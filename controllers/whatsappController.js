...
// ATUALIZADO: Agora recebe companyId para salvar no banco corretamente
const extractMessageData = (msg, sessionId, companyId) => {
    if (!msg.message) return null;
    if (msg.message.protocolMessage || msg.message.senderKeyDistributionMessage) return null;
    ...
    return {
        session_id: sessionId,
        company_id: companyId, // <--- ADICIONADO: Vínculo vital para o frontend achar a msg
        remote_jid: remoteJid,
        from_me: fromMe,
        content: content,
        message_type: 'text',
...
// ATUALIZADO: Agora recebe companyId
const upsertContact = async (jid, sock, pushName = null, companyId) => {
    try {
        ...
        const contactData = {
            jid: jid,
            company_id: companyId, // <--- ADICIONADO
            profile_pic_url: profilePicUrl,
            updated_at: new Date()
        };
        ...
        // Nota: Se a tabela contacts tiver unique apenas no jid, isso sobrescreve. 
        // Idealmente unique seria (jid, company_id) para multi-tenant real.
        await supabase.from('contacts').upsert(contactData, { onConflict: 'jid' });
    } catch (err) {
...
export const startSession = async (sessionId, companyId) => {
  ...
  // --- ATUALIZADO: Passando companyId para o histórico ---
  sock.ev.on("messaging-history.set", async ({ messages, contacts }) => {
      console.log(`[HISTORY] Processando ${messages.length} mensagens e ${contacts?.length || 0} contatos...`);
      // Correção aqui: passando companyId
      const formattedMessages = messages.map(msg => extractMessageData(msg, sessionId, companyId)).filter(Boolean);
      await saveMessagesBatch(formattedMessages);
      ...
      if (contacts) {
         const contactBatch = contacts.map(c => ({
             jid: c.id,
             company_id: companyId, // Correção aqui
             name: c.name || c.notify || c.verifiedName,
             push_name: c.notify
         }));
         const BATCH = 50;
  ...
  sock.ev.on("messages.upsert", async ({ messages }) => {
    ...
    if (formattedMessages.length > 0) {
        console.log(`[MSG] Recebida para empresa ${companyId}: ${formattedMessages[0].content.substring(0, 20)}...`);
        
        // Log de erro para debug no Render
        const { error } = await supabase.from('messages').insert(formattedMessages);
        if (error) console.error("Erro ao inserir mensagem:", error);
